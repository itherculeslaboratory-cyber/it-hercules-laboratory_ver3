// CL-04: 76-route matrix ↔ deny-by-default 照合 (design-c2 §2).
...
//   (iii) row count === 76. Lineage: base 68 (route-matrix.csv header comment) →
...
//        protected)= 71 → obs-capture レーン(V3-OBS-20 棚/場所QR)が +1 route
//        (infra-route-080: POST /placements/{placement_id}/qr・protected)= 72 →
//        obs-capture レーン(V3-OBS-72 研究室環境コンテキスト)が +3 route
//        (infra-route-081..083: POST/GET .../lab-environment・GET individuals/
//        {id}/lab-environment・全 protected)= 75 → obs-capture レーン(V3-OBS-61
//        自然言語フリーテキスト解析)が +1 route(infra-route-084: POST
//        /observation/parse-freetext・protected)= 76。
...
describe("CL-04 route matrix (76 rows)", () => {
  it("has exactly 76 route rows", () => {
