...
describe("T-66 GET /individuals/pedigree-links(宇宙面用・全個体血統エッジ一括投影)", () => {
  it("本人所有の個体間の親子リンクのみ返す(child/parent/roleの3値)", async () => {
    const { env } = ctx();
    const sire = await createInd(env, { local_label_text: "sire" });
    const dam = await createInd(env, { local_label_text: "dam" });
    const child = await createInd(env, { local_label_text: "child" });
    ...
    expect(body.links.sort((a, b) => a.parent_role.localeCompare(b.parent_role))).toEqual([
      { child_id: child, parent_id: dam, parent_role: "dam" },
      { child_id: child, parent_id: sire, parent_role: "sire" },
    ]);
  });

  it("血統リンクが無い個体しかいない場合は空配列(壊れない)", async () => {
    const { env } = ctx();
    await createInd(env, { local_label_text: "alone" });
    const res = await get("/api/v1/individuals/pedigree-links", env);
    expect(res.status).toBe(200);
    expect((await res.json()) as { links: unknown[] }).toEqual({ links: [] });
  });

  it("他 actor の血統は含まれない(透明性の文化と衝突しないスコープ分離)", async () => {
    const { env } = ctx();
    const aliceAuth = { Authorization: `Bearer ${await issueSessionToken("alice", SESSION_SECRET)}` };
    const aliceParent = await (
      await app.request(
        "/api/v1/individuals",
        { method: "POST", headers: { ...aliceAuth, ...JSON_HEADERS }, body: JSON.stringify({ local_label_text: "alice-parent" }) },
        env,
      )
    ).json();
    const aliceChild = await (
      await app.request(
        "/api/v1/individuals",
        { method: "POST", headers: { ...aliceAuth, ...JSON_HEADERS }, body: JSON.stringify({ local_label_text: "alice-child" }) },
        env,
      )
    ).json();
    await app.request(
      `/api/v1/individuals/${(aliceChild as { individual_id: string }).individual_id}/parents`,
      {
        method: "POST",
        headers: { ...aliceAuth, ...JSON_HEADERS },
        body: JSON.stringify({ parent_id: (aliceParent as { individual_id: string }).individual_id, parent_role: "sire" }),
      },
      env,
    );
    // dev(本人)は何も持っていない — bob 視点(devトークン)の一覧に alice の血統が漏れない。
    const res = await get("/api/v1/individuals/pedigree-links", env);
    expect((await res.json()) as { links: unknown[] }).toEqual({ links: [] });
  });
