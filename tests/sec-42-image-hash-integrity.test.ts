...
// gov-no-automod.test.ts と同型の負の回帰テストで固定する。市場出品写真(market-routes.ts
// POST /market/listings/:listing_id/photo)は「公式観測」ではなく別データ領域(出品商品写真)
// であり、observation/upload と同型パターンで独自に sha256 を都度算出・付与済み(C8波
// 86dfb03・本ファイルとは無関係に既存実装)。この構造ガードは公式観測画像の単一経路のみを
// 対象とするため既知の許容先として明示する。
...
describe("V3-SEC-42 single official image ingest path (negative regression)", () => {
  ...
  it("only observation-routes.ts (+ market-routes.ts listing photos, a distinct pre-existing non-observation domain) stores a media/photo blob via formData()+putBlob", () => {
    // ponytail: market-routes.ts は「公式観測」ではない出品写真の独立経路(既存実装・
    // 独自 sha256 付与済み)。この構造ガードの対象は公式観測画像の単一経路のみ。
    const KNOWN_NON_OBSERVATION_PHOTO_ROUTES = ["market-routes.ts"];
    const offenders = sources
      .filter((s) => /formData\(\)/.test(s.text) && /putBlob\(/.test(s.text))
      .map((s) => s.file);
    expect(offenders.sort()).toEqual(
      ["observation-routes.ts", ...KNOWN_NON_OBSERVATION_PHOTO_ROUTES].sort(),
    );
  });
