...
test("knowledge-thread: 返信+投稿ごと賛否ボタン+kebab展開+解決マーク", async ({ page }) => {
  await devLogin(page);
  ...
  await page.getByLabel("返信本文 *").fill("これはUIレビュー撮影用の返信です");
  await page.getByRole("button", { name: "返信する" }).click();
  await page.waitForLoadState("networkidle");
  await expect(page.getByText("これはUIレビュー撮影用の返信です")).toBeVisible();
