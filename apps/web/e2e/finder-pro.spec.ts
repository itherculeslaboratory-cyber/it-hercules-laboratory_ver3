...
test("caseB7実物: home→finder.html→sort→行選択→宇宙で見る→血統発光→詳細画面遷移", async ({ page, context }) => {
  ...
  // 1. home → 「個体を探す」(home v2・R112 90点: href直リンク) → finder.html。
  //    旧ラベル「理想の個体を探す」は home 完成予想図v2の主な行き先カードで
  //    「個体を探す」に更新された(c9-home-forecast-v2.html primary-card ind)。
  await page.goto(`${WEB}/`);
  await page.getByRole("link", { name: "個体を探す" }).click();
  ...
  await expect(uni.getByRole("heading", { name: /この子の「今」と「物語」/ })).toBeVisible();
  await uni.waitForLoadState("networkidle");
  await expect(uni.locator(".id-name")).toContainText(seed.sireLabel);
});
