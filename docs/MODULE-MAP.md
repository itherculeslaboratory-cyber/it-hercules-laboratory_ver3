...
★**訂正履歴(2026-07-31 第21回裁定・T1・カード`R0731-1ae43d-aip83scope-2026-07-31`AIP83-1=x)**: 本節の見出しは従来「既存97ファイルはリネームしない・新設のみ適用」だったが、この限定はV3-AIP-83裁定文(「全機能を api/logic/ui/page_info/threads の統一フォルダ構成で作る点は維持」)を狭める逸脱であり、ユーザーが事後承認せず差し戻した(逐語「大した手間じゃないでしょう。リネームとimport修整だけなんだから。」)。よって**全機能(既存97ファイル含む)を対象とする**よう訂正する。

★**実測結果(2026-07-31 lane-implement T1)**: `apps/api/src` 配下の実ファイル(`Get-ChildItem -File`実測=115本)を全件棚卸しし、下表「艦別・書いてよい場所」の17ドメインglobと突き合わせたところ、**既存ファイルは全件が命名規則(`<domain>-routes.ts`/`<domain>-constants.ts`)に既に準拠していた**(この命名規則自体が既存ファイル名から逆算して作られたものであるため)。したがって「新設のみ」制限が実際にコードへ及ぼした影響は0件であり、**リネーム対象のファイルは無かった**(実測=リネームすべき不一致ファイルはゼロ件。ドメイン未確定の11ファイル(下記「tests/の未帰属ファイルについて」節参照)は命名規則自体には準拠しているが、17ドメインのどれに属するかが未確定という別問題であり、今回の「新設のみ→全機能」の範囲訂正とは別論点として従来どおり未確定のまま残す)。

新規ドメインを1本追加する場合も、既存ドメインと同じく、以下5つを対にする(全部揃えなくてよい。必要なものだけ作る):
