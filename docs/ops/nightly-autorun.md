...
date: "2026-07-17"
status: active
requirement_ids: [V3-AIP-96, V3-AIP-98, V3-AIP-78]
---
...
## 夜間/週次実行パックへの精度最大化分割(V3-AIP-78)

「大量タスクを夜間overnight/週次実行パックとしてAI(Auto余力)に切り分け精度を最大化する。ただし完全
無人ワンクリック全自動は禁止し、迷う1割は人間ゲートを維持したうえでバッチ実行する」— 既存の
`runner.ps1` 分割単位がそのままこの要件の実装:

| 要件の概念 | runner.ps1 側の実装 |
|---|---|
| タスクを実行パックへ切り分け | `D:\claude\00-hq\night-tasks\*.md` 単位で1タスク=1ファイル。ランナーは毎回**1本だけ**選定して実行(精度最大化 = 一度に1本へ集中) |
| 完全無人ワンクリック全自動の禁止 | `enabled` 既定 false・R-6記名同意必須。連続成果ゼロ2回で自動停止(`zero_result_stop`)。V3-AIP-31のauto-gate-bypass機械ガード(`scripts/check-danger-code.mjs`)と同じ思想 |
| 迷う1割は人間ゲート維持 | ランナーが自律実行できない・判断が割れるタスクは `morning-review.md` の朝レビュースタックへ積まれ、人間が OK/NG する(Stage 0)。矛盾・不明点は保留として記録し(`docs/planning/rulings/`)、勝手に補完しない |
| 週次実行 | 週間グリッド(V3-AIP-98)の `auto` slot が実質的に「毎日 + 週次パターン」を担う。専用の週次バッチ機構は別途持たない(重複実装を避ける) |

repo側のコード変更は不要(パック分割・停止条件はHQ側 `runner.ps1` の既存ロジックがそのまま満たす)。
