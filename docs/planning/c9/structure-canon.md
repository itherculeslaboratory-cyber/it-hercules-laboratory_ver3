...
| 選択の粒度 | 階層: ドメイン→**学術分類(種→亜種粒度まで)**→**血統(育種ブランド)**。**どの深さでも選べる**。複数選択可+「すべて」(R144の三層定義=§1c) |
...
### 1c. 「観測対象」の三層定義(R144・2026-07-19ユーザー確定・HDR-1/HDR-2○)

ユーザーが観測対象を三層で定義した(逐語要旨・カード`c9-header-selector-scope`user_ruling)。ヘッダーセレクタ・全スコープ設計はこの定義に従う:

1. **学術分類(taxonomy)**: 種 → **亜種粒度まで**(例: ヘラクレスだけで亜種15種類・基亜種の学名 *Dynastes hercules hercules*)。観測を正確にするための情報の正確な分類。**スコープは昆虫に限らず全生物・無機物**(汎用観測プラットフォーム)。識別子はtaxon系(species/亜種)。
2. **血統(lineage)= 育種ブランド**: 「ブランド名みたいなもの」。飼育者が自分の系譜を「王シリーズ/玉シリーズ/彩シリーズ」のように整理する育種ラインの通称(=`lineage_id`自由タグ・V3-IND-34)。「天皇の血筋みたいなもん」。**HDR-2○=ヘッダーの『系統/血統(DHH)』絞り込みはこの`lineage_id`(育種ブランドタグ)で行う**(分類学上位でも実血統エッジでもない)。
3. **実の親子エッジ(cross_parent)**: 血統ブランド(層2)を管理するために必要な情報として**データ保持する**(ヘッダーの絞り込みには使わないが、系譜整理の土台)。**既存 V3-IND-01(親個体に父sire・母dam IDを記録)+V3-MKT-29(所有者移転時に生涯データを一本の系譜へ不可逆連結)で保持済み**=新規採番不要。血統ブランド(層2の`lineage_id`)と実血統(層3のsire/dam)は別軸として併存する。

**HDR-1○(実装方針)**: species/lineage_idが実在する3ドメイン(個体/クラッチ/観測capture)でヘッダーセレクタを先行実装し、市場/広場/研究は種族参照スキーマ設計(SW-1と同型)後に段階拡張する。「見た目だけ共有chrome」での完了報告は禁止(A1#4)。詳細スコープ=`00-hq/kits/ihl-final-run/`ヘッダーセレクタ調査(species実在3ドメイン・共有filter基盤ゼロ・識別子3系統)。
