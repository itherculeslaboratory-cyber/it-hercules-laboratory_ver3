...
現在地の最新は **§11(2026-07-19 完走)を読め=main=`66c9ed6`(doc tip `0f66b31`)・5+1統合波(mkt+ENTRY/ME/FORK/IND+fork-ab)+wave-obs-r4 まで全てmain統合済み**。§1〜§10 は 2026-07-18〜19 の進行ログ(歴史)であり、現在地判定には使わない。
最初の一手: **§11 の「申し送り(次スライス)」9項目を確認**(mkt4画面のhome入口配置=要ユーザー相談 / obs-entry item正規化 / Puck THIRD-PARTY表記 / E2E環境debt / c9-integrate worktreeクリーンアップ 等)。統合波は完了、残るは各申し送りの別ラウンド裁定。
```

## §1 現在地(歴史断面 = 2026-07-18クローズ時点。**最新は §11 = main `66c9ed6` / doc tip `0f66b31`**)

> ⚠ **本§1〜§8は 2026-07-18 クローズ時点の記述(main=`5ca805a`当時)。現在地の判定には §11 を読め**。以後 main は `5ca805a`→§10(総力戦ラン `d3283cb`/home v2 `842a71d`/ヘッダーセレクタ `1ab47d6`)→§11(5+1統合波 `84f6dba`+wave-obs-r4 `66c9ed6`)へ進行し、**構造正本v2・home v2・ヘッダー観測対象セレクタ・KNW種族の本・5+1統合波(全ゾーン)まで main 統合済み**。
