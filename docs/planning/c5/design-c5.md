...
2. **`ihl.research.citation.v1`** — `schemas/events/citation.schema.json`。**storage key はステータス/スナップショット込みの合成**: `sha1(content_id|provider_actor_id|source_session_id|status|match_snapshot…)` → `truth/ihl.research.citation.v1/<storage_key>.json`（同一提供の再 put=409・status=updated は別 storage key で別イベント append）。`citation_id`=canonical `sha1(content_id|provider_actor_id|source_session_id)` は 3 要素グルーピング識別子で **storage key とは別物**（3 要素だけを storage key にすると status=updated が provided と衝突し 409 になり「不足キー充足で別イベント append」が成立しないため合成キーで分離・批評家 minor）。envelope.id は別途 ulid()。`content_id`, `provider_actor_id`, `source_session_id`, `contribution_points`(int≥0・**サーバ固定の派生値**), `anonymous`(bool), `match_snapshot`(充足キー列), `status` enum=`["provided","updated"]`, `created_at`, `schema_version`。不足キー充足時の更新は status=updated の**別イベント append**（UPDATE 禁止）。
...
タグは**新スキーマを作らず** frozen `tag-event.schema.json` を再利用。envelope type=`ihl.research.tag_event.v1`・dataschema=`schemas/frozen/tag-event.schema.json`・`target_type="cross"`・`tag_type ∈ {system,ai,user}`・`source_type ∈ {ui,ai,user}`。ただし**人手経路 `POST /research/content/:id/tags` は `tag_type ∈ {ai,user}` のみ受理し `system` は 400**（system 層は「自動編集不可」V3-WIK-14 で agent/batch 経路のみが発行。人手 system 投稿は RAG_PRIORITY 最上位を悪用した検索順位操作口になる・批評家 minor）。
...
台帳連携（PPR-18・批評家 major 反映）: 観測 append 時のカルマ付与フックは C4 で保留された配線 — **本クラスタで citation 生成時に `grantPlatinum(s, providerActorId, amount, "contribution_rebate")`（ledger-routes.ts 既存関数）のみを呼ぶ**（貢献ポイント→プラチナ報酬）。**`amount` はサーバ固定 `CONTRIBUTION_POINTS_PER_CITATION`**（クライアント `contribution_points` は信用しない＝付与額をクライアントが選べると spendable/投票重み付き通貨の無制限鋳造になるため・批評家 major）。さらに**自己引用（`provider_actor_id == セッション principal`）は 400 で拒否**（自己報酬発券口を塞ぐ）。session は署名付きステートレスで所有者照合先の常駐 store が無いため source_session_id 所有検証は session 台帳導入波に defer（残余=共謀ペアの 1pt farm は自己引用禁止＋固定額で最小化）。**`grantKarmaCountIncrease` は呼ばない** — 同関数はカルマ count(=違反回数)を進め fibPenalty を value 層に -penalty で append する**懲罰関数**（reason_code=dispute/fee_unpaid 等）であり、貢献者を減点し PPR-18『貢献ポイント付与』の意図と正反対になる。この台帳に正の count 報酬関数は存在しないため、正の報酬は grantPlatinum に一本化する。付与関数は再利用し再実装しない。
