...
補足：Dockerは写真を撮った後、embeddingや写真の解析処理などに使われる方がメインの想定になりつつあります。
写真解析に必要な計算資源はユーザーの方で担保したい。スマホで済むなら、最高。さらに高度な、解析とか収まらないなら、Docker。
...
補足：その人が何言語を種言語として選択しているかのほうが重要な気がしますね。
結局住んでいるとこより、どの言語文化圏にいるかのほうが影響受けそうですよね？

ただ、国によって規制やrule、考え方が違うので、　行政介入権利とか　国でフィルタリングしたいよね？　この国では合法、この国では違法ってあるから、だから国籍選択は欲しいよね。
...
補足；
商用3%はお金の話なので。あってますし、スムーズ。
フォーク10%は貢献度の話です。

このフォークされたユーザーDのテンプレートが使われたとして、

それに使われている部品(コンポーネント)や、処理（技術者、開発者）、元テンプレートの人に、　Dに付与される貢献度の10%くらい、入ってもいいよなって。そしたら、ほんとにいいものを作った人が得をする。インセンティブが高まる。
コピーしてパクられてもそのほうが得ってなるようにしたい。
