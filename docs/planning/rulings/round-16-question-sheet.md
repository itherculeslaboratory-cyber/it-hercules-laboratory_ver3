...
Q-PAY-01は回答済み(2026-07-16: PayPay統一で確定・GMOあおぞらは個人事業主に本番API非提供と公式確認)。以下は実現方法の追加裁定4問+進め方1問+既存3問。

### [Q-PAY-02] 決済まわりの外部問い合わせ(人間アクション)の進め方。対象: ①KOMOJU仮審査照会(昆虫マーケット運営+システム利用料徴収の業態開示) ②PayPay OPAのプラットフォーム業態・オンライン専業可否の照会 ③PayPay銀行Open API(参照系・自社口座のみ)の要件照会 ④弁護士へのPTコイン前払式支払手段該当性の確認

いずれも確認が済むまで該当機能の本番接続には着手しません(設計+sandboxまで先行)。

- **★推奨** AIが全問い合わせ文面を起草→ユーザーが送信
- ユーザーが後日自分で問い合わせ
- 本番契約直前まで先送り(設計だけ進める)

推奨理由: 文面起草まで自走し、送信だけ人間ゲートに残すのが最速。

材料: `docs/planning/b2-research/research-paypay-unification.md`

### [Q-PAY-03] IHL徴収層(5%システム維持費)のPayPay決済の実装経路。

追加調査の結論: 買主→売主の代金はPayPay送金にできない(規約で営利利用が禁止・一次情報で確定)ため、PayPay化できるのはIHL自身の徴収層のみ。経路比較: (a)中間PSP KOMOJU=個人事業主OK・サンドボックス即日・PayPayオンライン手数料3.5%(5%維持費に対してなので成約額比0.175%) (b)PayPay OPA直接=1.98%(成約額比0.099%)だが本番接続まで約2ヶ月+プラットフォーム業態の審査通過が不確実。手数料差は成約額比0.1%未満と僅少。

- **★推奨** 段階併用: Phase1=KOMOJUで即ローンチ + Phase2=PayPay OPA直接加盟を並行申請し、承認後に低料率へ任意移行
- OPA直接のみ(審査約2ヶ月を待ってからローンチ・実効最安)
- KOMOJUのみ(OPA申請はしない)
- 銀行振込+会計APIかませ(PayPay決済は使わない)

推奨理由: ローンチをOPA審査でブロックせず、料率最適化は後から可逆に行える。

材料: `docs/planning/b2-research/research-paypay-unification.md §1,§3`
...
### [Q-PAY-05] [設計確定] 買主→売主のP2P代金(銀行振込)をIHLがどう扱うか。現行ワイヤーF3は『振込→自動検知』と記述していますが、P2P代金はIHLの口座を通らないため技術的に検知不能です。

(a)なら資金移動業リスクゼロ・GMOあおぞら完全解約が可能・U-code(CL-11)はP2P用途では不要化(5%徴収はmerchantPaymentId照合に置換)。(b)はIHLが代金を預かるエスクロー型となり資金移動業登録が必要になる可能性が高い。

- **★推奨** (a) IHL非関与を明確化: 買主が売主の口座へ直接振込→売主が自分の銀行で入金確認→アプリで受取確定を押す(IHLは照合せず状態遷移のみ管理)
- (b) IHL口座経由で代金を預かる(エスクロー型・資金移動業リスク大)
- (c) (a)を採用し、将来オプションとして売主向け照合支援(売主自身の会計API連携等)を第2波で検討

推奨理由: (c)を選んでも(a)と同じ実装で開始できます(第2波検討のフラグが付くだけ)。

材料: `docs/planning/b2-research/research-paypay-unification.md §2-C` / `docs/planning/c7/wireframes-core5.md F3`

### [Q-PAY-06] PTコイン(プラチナ)チャージ機能の実装凍結。

調査で二重ブロッカーが判明: ①要件矛盾=V3-MKT-38『プラチナは金銭購入不可』とコードのpt_topupが正面衝突(V3-KRM-10はPTとプラチナを別概念と定義) ②PayPay加盟店規約がプリペイド/有価証券販売を原則禁止(個別承諾必須)+前払式支払手段(資金決済法)該当性は弁護士確認事項。5%維持費の徴収には影響しません(通常のサービス利用料)。

- **★推奨** 凍結承認: 矛盾裁定+弁護士確認が済むまでPTチャージは実装しない(5%徴収のみ先行)
- 今この場で方針を裁定する(コメント欄で指定)

材料: `docs/planning/b2-research/research-paypay-unification.md §2-B` / `01-requirements/registry.json V3-MKT-38/V3-KRM-10`
