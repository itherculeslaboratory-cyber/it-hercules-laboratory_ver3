...
ラン全体の方向が変わる5問。特に Q-PAY-01 は元指示と推奨が異なるため明示裁定をお願いします。

### [Q-PAY-01] GMOあおぞら→PayPay統一の実現方式をどれにするか。

Web一次情報調査の結論: (1)『PayPay』は別会社2つ — PayPay銀行(旧ジャパンネット銀行)とPayPay株式会社(QR決済/OPA)。(2)PayPay銀行には個人事業主が単独契約できるリアルタイム入金Webhookが確認できず(小口向け振込専用口座はAPI通知なし+新規契約停止中)、sunabar相当のサンドボックスも無く、API料金非公開・最短6〜7週の個別審査 = 現行のGMO入金照合(検証済)をPayPay銀行では再現できない。(3)PayPay決済(OPA)は個人事業主可・開発サンドボックスあり・手数料1.98%(取引額比例)・昆虫生体の直接禁止規定なし(昆虫店『極』『クワガタ横丁』の導入実例あり)。ただし C2C分配/生体審査承認/資金決済法該当性 の3点が未確認(P0)。

- **★推奨** 方式D: 決済のみPayPay追加・銀行はGMOあおぞら維持(非破壊・10年コスト最小)
- 方式B: PayPay決済(OPA)へ一本化・銀行振込は廃止
- 方式A: PayPay銀行へ全面移行(調査上は実現困難)
- 現状維持: PayPay導入自体を見送り

推奨理由: ※元指示『あおぞらGMOやめます』と異なる推奨です。GMO口座を解約すると検証済みの入金照合基盤を失い、PayPay銀行側に個人事業主向けの代替がありません。ここだけは明示の裁定をお願いします。

材料: `docs/planning/c8/PLAN-c8-full-run.md §3(PayPay調査要約)`

### [Q-PAY-02] PayPay加盟店のP0未確認3点(①昆虫生体の審査承認 ②C2C分配=複数出品者の代金徴収・5%控除・分配の規約可否 ③資金決済法(資金移動業)該当性)の確認方法。問い合わせ送信は人間アクションです。

いずれの回答でも、確認が済むまでPayPay決済の本番接続には着手しません(設計+sandbox検証まで)。③は最終的に弁護士確認が推奨されています。

- **★推奨** AIが問い合わせ文面を起草→ユーザーが送信
- ユーザーが後日自分で問い合わせ
- 確認は本番契約直前まで先送り(設計だけ進める)

推奨理由: 文面起草まで自走し、送信だけ人間ゲートに残すのが最速。

材料: `docs/planning/c8/PLAN-c8-full-run.md §3`
