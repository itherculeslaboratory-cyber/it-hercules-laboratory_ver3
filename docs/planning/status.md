...
**C8 進捗（2026-07-17 実測・`docs/planning/c8/progress.json` 集計）**: 全体 334件中 done 125(37%)・in_progress 24・blocked 3・todo 182。**第1波必達(required) 184件中 done 122(66.3%)**・in_progress 24・blocked 3・todo 35。**第2波ベストエフォート(best-effort) 150件中 done 3(2.0%)**・todo 147。詳細は `docs/planning/c8/progress.md`(生成物)。要件ID↔TCカバレッジは `docs/planning/c8/tc-coverage-c8.md` に機械集計（必達doneのうち機械的にTC根拠を確認できた分＝91/122=74.6%、根拠付きgreen率100%）。本ラン実測: `npm run lint` 20 GATE 全OK／`npm test` 165 files・1395 passed・0 failed／`pytest -q` 49 passed・1 skipped（`npm run e2e` は未再実行・前回実測60/60 greenは`HANDOFF-c8-session2.md`§0参照）。
