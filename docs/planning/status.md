...
**C8 進捗（2026-07-17 実測・セッション2最終・`docs/planning/c8/progress.json` 集計）**: 全体 334件中 done 158(47.3%)・in_progress 26・blocked 3・todo 147。**第1波必達(required) 184件中 done 155(84.2%)**・in_progress 26・blocked 3・**todo 0**（`HANDOFF-c8-session2.md` §7 完走定義①「required 全件が done/in_progress(残余理由をnoteに明記)/blocked(裁定・照会待ちを明記)のいずれかに分類され todo が0件」を本ランで達成。in_progress 26件は全件 `note` に残余理由を記入済み・空欄0件を機械確認）。**第2波ベストエフォート(best-effort) 150件中 done 3(2.0%)**・todo 147（本ラン対象外・前回と不変）。前回実測(commit 363cc5d時点)比 required done 125→155(+30)・todo 33→0(-33)はセッション2継続実装（UI磨き第2弾・L4残クラスタ消化・市場残route再設計等)による。詳細は `docs/planning/c8/progress.md`(生成物)。要件ID↔TCカバレッジは `docs/planning/c8/tc-coverage-c8.md` に機械集計（必達doneのうち機械的にTC根拠を確認できた分＝122/155=78.7%、根拠付きgreen率100%）。本ラン実測: `npm run lint` 21 GATE 全OK／`npm test` 165→182 files(+17)・1395→1525 tests(+130)・0 failed／`pytest -q` 49 passed・1 skipped（前回と同一）／`npm run e2e -w apps/web` は本ランでフル実行し**174/174 green を実測確認**(前回版までは前回実測の借用のみだったが、本版で自ら検証・解消。1回目試行は外形タイムアウトで打ち切られ末尾1本が見かけ上失敗したが、フル再実行で174/174 green — プロセス強制終了の副作用であり実バグではない)。
...
**外部照会 回答受領済(2026-07-17)**: PAY.JP=Platformプラットフォーマー契約は法人限定・本サービスの昆虫売買は取扱不可(→Platformオプション退役・2026-08-07裁定)。PayPay=決済代行会社経由必須(→8/9スコープ外)。
...
| PAY.JP 本番アカウント申込 | 個人事業主・昆虫マーケット運営+システム維持費5%徴収の用途で申込。**照会回答受領(2026-07-17)。本番アカウント申込は未実施(5%徴収に必要・人間ゲート)** | 照会回答受領(2026-07-17)。本番アカウント申込は**未実施** |
| PayPay OPA 照会・並行申請 | 固定店舗なし個人事業主の契約可否・実料率・審査期間を照会のうえ並行申請。**照会回答受領(2026-07-17)。決済代行会社経由必須のため8/9スコープ外(2026-08-07裁定)** | 照会回答受領(2026-07-17)。決済代行会社経由必須のため8/9スコープ外(2026-08-07裁定) |
