...
**C7(次セッション)= 実画面検証 → 最終打鍵チェック準備が最優先**(第13回裁定・V3-AIP-101)。kickoff 正本: `docs/planning/c7/HANDOFF-c7-2026-07-11.md`。

1. **全画面の実ブラウザ走査**(V3-AIP-101): 全 screen-def を実起動・実操作で検証し、モック/未配線/エラーの不在をスクリーンショット付き実測レポートに → 安心状態にしてから最終打鍵チェックを依頼
