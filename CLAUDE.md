...
3. **数字は必ず自分で数える**: `01-requirements/README.md` は「716 件」と書いたまま陳腐化している（実数 758）。README の数字を信じるな、ファイルを数えろ。
...
2. **ユースケース起点** — 「ユーザーはなぜこの画面に来るか」から設計。要件レジストリ（749件）は検算用であり起点にしない。
